-- 다시봄 — 세션 스냅샷 원자적 저장 RPC.
--
-- Supabase REST(PostgREST)는 `.from(table).insert()/delete()` 호출을 각각 별도
-- 네트워크 요청으로 커밋한다 — 자식 테이블 9개를 "삭제 후 재삽입"하는 전체 스냅샷
-- 저장을 여러 번의 REST 호출로 하면 중간에 실패했을 때 부분 반영이 남는다. 이는
-- CLAUDE.md 원칙 7(입력 유실 금지)을 정면으로 어긴다. 그래서 전체 반영을 Postgres
-- 함수 하나로 묶어 단일 트랜잭션으로 원자적으로 처리한다.
--
-- SECURITY INVOKER — 호출자의 RLS 권한 그대로 적용된다(관리자 우회 없음). 다른
-- 사용자의 session_id를 넣으려는 시도는 각 INSERT/UPDATE의 RLS 정책이 그대로 막는다.
...
begin
  ...
  select (x->>'id')::uuid, v_session_id, x->>'text', x->>'lens_type', x->>'author_type',
         (x->>'reframe_order')::int,
         case when x->'selected_elements' is null then null
              else array(select jsonb_array_elements_text(x->'selected_elements')) end
  from jsonb_array_elements(p_reframes) x;

  -- ai_feedbacks ↔ problem_definition_versions는 서로를 참조한다(순환).
  -- 자식(ai_feedbacks)을 먼저 지우고, 버전을 based_on_feedback_id 없이 다시 넣은 뒤,
  -- 피드백을 넣고, 마지막으로 based_on_feedback_id를 채운다.
  delete from public.ai_feedbacks where session_id = v_session_id;
...
grant execute on function public.save_training_session_snapshot(
