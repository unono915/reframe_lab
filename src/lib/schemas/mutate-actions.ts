...
export const mutateActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    action: z.literal("markPriorityQuestion"),
    args: priorityQuestionInputSchema,
  }),
  z.object({
  ...
  z.object({
    action: z.literal("submitExceptionReason"),
    // 예약 표식(`solo_mode`·`self_check_*`)을 이 경로로 쓸 수 없게 좁힌 스키마다.
    args: exceptionReasonInputSchema,
  }),
