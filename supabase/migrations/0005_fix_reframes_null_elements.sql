-- 버그 수정: jsonb null(키는 있지만 값이 JSON null)과 SQL NULL(키 자체가 없음)을
-- 혼동해 `x->'selected_elements' is null`이 JSON null을 걸러내지 못했다.
-- `array(select jsonb_array_elements_text('null'::jsonb))`는 "cannot extract
-- elements from a scalar" 에러를 낸다. 실제 RPC 수동 검증(2026-08-15) 중 발견.
