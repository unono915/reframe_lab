    ...
    >
      <Stack gap={6}>
        ...
        <Stack gap={3}>
          ...
          {/*
            PRD F-06은 "먼저 두 프레임을 작성한 후에만 추가 렌즈 힌트"를 원칙으로 두되
            **"막힘이 명시된 경우 단계적 힌트 예외를 허용한다"**고 덧붙인다. 이 화면은
            프레임이 하나일 때 이미 "더 떠오르지 않는다면 이유를 적어주세요"를 띄운다 —
            앱이 스스로 막힘을 인정한 시점이다. 그래서 하나부터 열어둔다.

            여는 조건은 서버가 쓰는 것과 같은 함수로 판정한다(`useStageHint`) — 화면이
            열어준 버튼이 서버에 거절당하는 어긋남을 예외 경로에서 실제로 겪었다.
          */}
